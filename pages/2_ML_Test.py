...
def load_models():
    try:
        for f in ["log_model.pkl", "rf_model.pkl", "xgb_model.pkl"]:
...
with col1:
    p1_1_disp = st.selectbox("P1 Card 1", display_options)
    p1_2_disp = st.selectbox("P1 Card 2", display_options)

with col2:
    p2_1_disp = st.selectbox("P2 Card 1", display_options)
    p2_2_disp = st.selectbox("P2 Card 2", display_options)

with col3:
    p3_1_disp = st.selectbox("P3 Card 1", display_options)
    p3_2_disp = st.selectbox("P3 Card 2", display_options)

st.markdown("## 🃏 Board")
b1,b2,b3,b4,b5 = st.columns(5)

with b1:
    f1_disp = st.selectbox("Flop 1", display_options)
with b2:
    f2_disp = st.selectbox("Flop 2", display_options)
with b3:
    f3_disp = st.selectbox("Flop 3", display_options)
with b4:
    turn_disp = st.selectbox("Turn", display_options)
with b5:
    river_disp = st.selectbox("River", ["🚫 Not Opened"]+display_options)
